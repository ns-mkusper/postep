...
function parseBenchmarkInteger(
...
export const UX_BENCHMARK_ITERATIONS = parseBenchmarkInteger(
  process.env.UX_BENCH_RUNS,
  50,
);
export const UX_BENCHMARK_WARMUPS = parseBenchmarkInteger(
  process.env.UX_BENCH_WARMUPS,
  5,
);
...
export function createLargeOrgCorpus(
  docCount = 100,
  sectionsPerDoc = 20,
): string[] {
  return Array.from({ length: docCount }, (_, docIdx) => {
    const day = String((docIdx % 28) + 1).padStart(2, "0");
    const sections = Array.from({ length: sectionsPerDoc }, (_, sectionIdx) => {
      const status =
        sectionIdx % 4 === 0
          ? "TODO"
          : sectionIdx % 4 === 1
            ...
            : sectionIdx % 4 === 2
              ...
              : "";
      const heading =
        `${"*".repeat((sectionIdx % 3) + 1)} ${status} Mobile section ${docIdx + 1}.${sectionIdx + 1}`.replace(
          /\s+/g,
          " ",
        );
      return `${heading} :mobile:perf:
SCHEDULED: <2026-07-${day} Tue +1d>
DEADLINE: <2026-08-${day} Wed 09:00>
- [ ] Render card ${sectionIdx + 1} without blocking gestures
- [X] Keep native bridge work off the JS thread
- Review [[id:large-${docIdx}-${sectionIdx}]][linked note]] and tags`;
    }).join("\n\n");
    return `#+TITLE: Large UX Sample ${docIdx + 1}
${sections}
`;
  });
}

export const largeOrgSamples = createLargeOrgCorpus();

export function syntheticOrgNodes(raw: string): SyntheticOrgNode[] {
  return raw.split("\n").flatMap((line, idx): SyntheticOrgNode[] => {
    const heading = line.match(
      /^(\*+)\s+(?:(TODO|WAITING|DONE)\s+)?(?:\[#([A-Z])\]\s+)?(.*?)(\s+:[^\s]+:)?$/,
    );
    if (heading) {
      return [
        {
          ...
          tags: (heading[5] ?? "").trim().split(":").filter(Boolean),
        },
      ];
    }
...
export function benchmark(name: string, fn: () => unknown): BenchmarkStats {
  ...
  const percentile = (rank: number) =>
    samples[
      Math.min(samples.length - 1, Math.floor((samples.length - 1) * rank))
    ];
...
export function assertMedianWithinBudget(
...
export function createEditorFrontendBenchmarkSuite<
  TNode,
  TProjection,
  TBlockViewModel,
>(frontend: EditorFrontendAdapter<TNode, TProjection, TBlockViewModel>): void {
  describe(`${frontend.name} editor frontend repeated UX benchmarks`, () => {
    it("projects rendered blocks inside repeated median budget", () => {
      const stats = benchmark(`${frontend.name}:blockProjection`, () =>
        orgSamples.flatMap((raw) =>
          frontend.createBlockViewModels(raw, frontend.syntheticNodes(raw), {
            outlineOnly: false,
            readerMode: true,
          }),
        ),
    ...
    it("projects large mobile corpus inside repeated median budget", () => {
      const stats = benchmark(`${frontend.name}:largeCorpusProjection`, () =>
        largeOrgSamples.flatMap((raw) =>
          frontend.createBlockViewModels(raw, frontend.syntheticNodes(raw), {
            outlineOnly: false,
            readerMode: true,
          }),
      ...
      console.log(formatStats(stats));
      assertMedianWithinBudget(
        stats,
        Math.max(frontend.budgets.projectionMs * 5, 60),
      );
    });
    ...
    it("moves individual org blocks inside repeated median budget", () => {
      ...
      const heading = frontend
        .syntheticNodes(raw)
        .find((node) => frontend.isHeading(node, "Agenda item"));
      assert.ok(heading, "expected agenda heading test node");
      const stats = benchmark(`${frontend.name}:blockMove`, () =>
        frontend.moveRawBlock(raw, heading, -1),
      );
      console.log(formatStats(stats));
    ...
    it("edits individual org blocks inside repeated median budget", () => {
      ...
      const listItem = frontend
        .syntheticNodes(raw)
        .find((node) => frontend.isListItem(node, "render pass"));
      assert.ok(listItem, "expected render-pass list item test node");
      const stats = benchmark(`${frontend.name}:blockEdit`, () =>
        frontend.updateRawBlock(
          raw,
          listItem,
          "- [X] complete app render pass",
        ),
      );
      console.log(formatStats(stats));
    ...
    it("generates read-mode projection after common toggles inside repeated median budget", () => {
      ...
      const stats = benchmark(`${frontend.name}:readModeProjection`, () =>
        frontend.nodesToProjection(fallbackRaw, nodes, {
          outlineOnly: false,
          readerMode: true,
        }),
      );
